...
# Padding must remain unchanged after reduction and consumed-slot clearing.
PADDING_SENTINEL = -123.0


# --------------------------------------------------------------------------
# plan builders. Plans are global [R, epn] tables, so every builder must be a
# deterministic function of (R, epn, seed) only — never of the local rank.
# --------------------------------------------------------------------------


def _plan_ring(R, epn, dev, seed=0):
...
def _plan_fan_in(R, epn, dev, seed=0):
    ...
    """
    assert 2 * R <= epn, f"fan_in plan needs epn >= 2R, got epn={epn}, R={R}"
    plan = torch.full((R, epn), -1, dtype=torch.int32)
    for dst in range(R):
...
def _plan_empty(R, epn, dev, seed=0):
    return torch.full((R, epn), -1, dtype=torch.int32).to(dev)


def _plan_asymmetric(R, epn, dev, seed=0):
...
def _plan_random(R, epn, dev, seed=0):
    g = torch.Generator()
    g.manual_seed(seed)
    live = torch.rand((R, epn), generator=g) < 0.6
    ids = torch.randint(0, R * epn, (R, epn), generator=g, dtype=torch.int32)
...
def _grad_values(rank, epn, H, Hp, dev):
    expert = (
...
def _slot_values(rank, epn, H, Hp, dev):
    slot = torch.arange(epn, dtype=torch.float32, device=dev).view(epn, 1, 1)
    row = torch.arange(H, dtype=torch.float32, device=dev).view(1, H, 1)
    col = torch.arange(Hp, dtype=torch.float32, device=dev).view(1, 1, Hp)
...
def _make_data_fns(mode, rank, epn, H, Hp, dev, seed):
    """Return (grads_fn, slot_fn). Both regenerate identical bits per call."""
    if mode == "affine":
        return (
            lambda: _grad_values(rank, epn, H, Hp, dev),
            lambda src: _slot_values(src, epn, H, Hp, dev),
        )
    assert mode == "randn", mode
    return (
        lambda: _randn((epn, H, Hp), 200_000 + 1_000 * seed + rank, dev),
        lambda src: _randn((epn, H, Hp), 100_000 + 1_000 * seed + src, dev),
    )
...
def _expected_for_rank(rank, R, epn, plan_cpu, grads_fn, slot_fn):
    expected = grads_fn()
    ...
    for src in range(R):
        ...
        for b in range(epn):
            ...
            if local_start <= expert < local_end:
                expected[expert - local_start].add_(slot_vals[b])
    return expected
...
def _verify(rank, R, epn, plan, grads_fn, slot_fn,
            local_expert_grads, reduce_buffers, label):
    plan_cpu = plan.cpu()

    expected_local = _expected_for_rank(rank, R, epn, plan_cpu, grads_fn, slot_fn)
    ...
    ok_grads = torch.equal(local_expert_grads, expected_local)
    ...
    ok = bool(ok_grads and zero_ok and unchanged_ok)
    if not ok:
        print(
            f"[rank {rank}] {label}: grads={ok_grads} "
...
CASES = [
    {
        "name": "single_tile",
        "epn": 32,  # 2 MiB fp32 rank chunk at 128x128.
        "H": 128,
        "Hp": 128,
        "num_sms": 8,
    },
    {
        "name": "multi_tile",
        "epn": 16,  # 2 MiB fp32 rank chunk at 256x128.
        "H": 256,
        "Hp": 128,
        "num_sms": 16,
    },
    ...
    # arithmetic in the compact local-grad addressing.
    {
    ...
    # work item, plus self-sends and same-src duplicate experts. epn=16 also
    # provides the fixed-width, VMM-aligned reduce chunk.
    {
    ...
    # H/Hp are enlarged solely to keep the epn=1 reduce chunk VMM-aligned.
    {
        "name": "epn1_num_sms1",
        "epn": 1,
        "H": 1024,
    ...
    # R*epn=64 (two 32-lane prescan chunks at R=4) with rounding randn data.
    {
        "name": "random_plan_s1",
        "epn": 16,  # 2 MiB fp32 rank chunk at 256x128.
        "H": 256,
    ...
    {
        "name": "strided_non_row_aligned_randn",
        "epn": 3,
        "H": 384,
        "Hp": 640,
        "num_sms": 2,
...
def _make_reduce_pool(rank, R, epn, H, Hp, extra_rank_pages=0, offset_bytes=0):
    stride_bytes = pad_to_granularity(epn * H * Hp * 4 + offset_bytes)
    stride_bytes += extra_rank_pages * pad_to_granularity(1)
    rank_stride = stride_bytes // 4
    backing = create_nvl_dist_tensor([rank_stride], torch.float32, rank, R).view(R, rank_stride)
...
def _verify_padding(rank, pool, backing, label):
    begin = pool.storage_offset()
    end = begin + pool[rank].numel()
    ok = bool((backing[rank, :begin] == PADDING_SENTINEL).all()) and bool(
        (backing[rank, end:] == PADDING_SENTINEL).all()
    )
    _assert_all_ranks(ok, rank, f"{label}: reduce pool padding was modified")
...
def run_case(rank, R, case):
    ...
    reduce_buffers, reduce_full = _make_reduce_pool(
        rank, R, epn, H, Hp,
        case.get("extra_rank_pages", 0),
        case.get("offset_bytes", 0)
    )
    plan = PLAN_BUILDERS[case.get("plan", "ring")](R, epn, dev, seed)
    grads_fn, slot_fn = _make_data_fns(
        case.get("data", "affine"), rank, epn, H, Hp, dev, seed)
    ...
    try:
        ...
        local_expert_grads = grads_fn().contiguous()
        reduce_full[rank].fill_(PADDING_SENTINEL)
        ...
        _verify(rank, R, epn, plan, grads_fn, slot_fn,
                local_expert_grads, reduce_buffers,
                f"{case['name']} grad_reduce failed")
        _verify_padding(rank, reduce_buffers, reduce_full, case["name"])

        if rank == 0:
            print(
                f"  [PASS] {case['name']}: R={R}, E={E}, epn={epn}, "
...
@pytest.mark.parametrize("shape", [(16, 256, 128), (3, 384, 640)], ids=["contiguous", "strided"])
def test_grad_reduce_repeated_launch(dist_env, shape):
    ...
    num_sms = 16
    E = R * epn
    reduce_buffers, reduce_full = _make_reduce_pool(rank, R, epn, H, Hp)
    ...
    try:
        ...
        for it, (plan_name, plan_seed) in enumerate(rounds):
            plan = PLAN_BUILDERS[plan_name](R, epn, dev, plan_seed)
            grads_fn, slot_fn = _make_data_fns(
                "randn", rank, epn, H, Hp, dev, seed=50 + it)

            local_expert_grads = grads_fn()
            reduce_full[rank].fill_(PADDING_SENTINEL)
            reduce_buffers[rank].copy_(slot_fn(rank))
            torch.cuda.synchronize()
            dist.barrier(device_ids=[local_device_index()])
            ...
            _verify_padding(rank, reduce_buffers, reduce_full, f"repeated_launch round {it}")
